...
t <- ggtree(hoxt, aes(x, y)) + geom_tree() + theme_tree()  + geom_tiplab(cex=2.5) + geom_label2(size=2.5,aes(subset = !isTip, label=label),nudge_x=0,nudge_y=.175,  label.padding = unit(0.05, "lines")) + ggplot2::xlim(0, max(6,0.389*(sum(hoxt$edge.length))-26.389 )) +      theme(plot.margin=unit(c(0,0,0,0), "mm"))
